(function(){
  ...
  $('#startButton').addEventListener('click',async()=>{
    ...
    active=true; startedAt=Date.now(); document.documentElement.classList.add('quiz-locked'); document.body.classList.add('test-active'); $('#startPanel').hidden=true; $('#testPanel').hidden=false; renderQuestion();
  ...
  document.addEventListener('selectstart',e=>{if(active&&!submitted){e.preventDefault();const selection=window.getSelection&&window.getSelection();if(selection)selection.removeAllRanges();}},true);
  ...
  document.addEventListener('dragstart',e=>{if(active&&!submitted)e.preventDefault();},true);
  document.addEventListener('contextmenu',e=>{if(active&&!submitted)e.preventDefault();},true);
  ['pointerup','touchend','mouseup'].forEach(type=>document.addEventListener(type,()=>{if(active&&!submitted){const selection=window.getSelection&&window.getSelection();if(selection)selection.removeAllRanges();}},true));
  document.addEventListener('keydown',e=>{if(active&&!submitted&&(e.ctrlKey||e.metaKey)&&['a','c','x','u','s'].includes(e.key.toLowerCase()))e.preventDefault();},true);
  ...
  function finishTest(){
    submitted=true;active=false;document.documentElement.classList.remove('quiz-locked');document.body.classList.remove('test-active');clearInterval(heartbeatTimer);
